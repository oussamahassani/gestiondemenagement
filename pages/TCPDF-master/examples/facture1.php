...
$hhh="05/05/1988";
				...
				require_once '../../../connect.php';
						$id=$_GET['id'] ;
						$idd=$_GET['dev'];
				 $req=mysql_query("select * from demande,client,facture where encaissement.id_devis=devis
.id_devis and demande.id_dem=demande.id_demande  and demande.id_client=client.id_client and demande.id_dem='$idd'");
				 while($result=mysql_fetch_array($req))


			{

				$type=$result['type_prestation'];
        $reste=$result['reste_encai'];
			   $total=$result['total_encai'];

				$title='Facture_'.$result['civilite'].''.$result['nom'].'_'.$result['prenom'].'.pdf';
...
<font size="14" color="#FFFFFF" >Date; : '.$date.'</font><br><br>
	<font size="14" color="#FFFFFF" ><b> encaisement &deg; : '.$id.'</b></font>
	</th>
    </tr>
  ...
  <tr>
	...
	<tr>
	   ...
	   <br><br>
	   <font size="11" color="#000">
	   TOTAL: '.$total.' <br> total encaisement: <b>'.number_format($ptva, 2, ",", "").'</b></font>
	   </td>
	   <td align="left">
	   <br><br><font size="11" color="#000">
	    reste:'.number_format($reste, 2, ",", "").' </font>
	   </td>
    </tr>
