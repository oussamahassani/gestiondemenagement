<?php	
	...
	$yourprenom=$_POST['lieua'];
	$volume=$_POST['volume'];
    $villeddep=$_POST['lieub'];
    $villearivee=$_POST['formule'];	
	$volume1=$volume/2;
	...
	$today = date("Y-m-d");  
	$date_etab=$today;
	$villeddep=str_replace("'", "&acute;",$villeddep);

	

	//$remarque=str_replace("'", "&acute;",$remarque);
	$req2=mysql_query("insert into client(clientB2B,type_client,nom,prenom) values (0,'1','$yourname','$yourprenom')");
	
	
	$idclient=mysql_insert_id();	    
	$req3=mysql_query("insert into demande(id_type,type_facturation, id_source,id_client,etablie_le,ville_dep,date_dep,ville_arr,volume) 
	                              values ('$idType','11','$source','$idclient','$date_etab','$villeddep','$dates','$villearivee','$volume1')");
