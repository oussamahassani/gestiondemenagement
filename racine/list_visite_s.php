...
if (isset($_SESSION['id']) && isset($_SESSION['nom_com'])) {
	if (isset($_GET['date_vis']) && isset($_GET['id_statut_vis'])) {
		...
		$d_vis 			= explode('-', $date_vis);
		$y_visite 		= current($d_vis);
		$m_visite 		= $d_vis[1];
		$y_m_visite 	= $y_visite . '-' . $m_visite;

		$dernier_jour_mois 		= date($date_vis, strtotime('last day of this month', time()));
		// $premier_jour_mois 		= date('Y-m-01');
		$premier_jour_mois 		= date($y_m_visite.'-01');
		...
		$query_list_ca = "
			...
			WHERE
				...
				AND (date(v.date_vis) BETWEEN '$premier_jour_mois' AND '$dernier_jour_mois')
		";
