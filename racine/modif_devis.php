...
<div class="content-wrapper">
  <div class="page-title">
      <div class="row">
          <div class="col-sm-6">
          <h4 class="mb-0">Modification devis  N° <?php echo $id; ?>- demande N° <?php echo $result['id_dem'];?></h4>
 ...
 <div class="row">   
      <div class="col-xl-12 mb-30">     
        <div class="card card-statistics h-100"> 
          <div class="card-body">
<form id="form" action="inc/insert_modif_devis.php" enctype="multipart/form-data" method="post" class="validateform" name="contact" >
<section>
...
<div class="form-inline" > 
  ...
  <span class="input-group-addon">
                ...
                <input name="date_etab_devis"  class="form-control" type='text' required="required" value="<?php $date=$result['devis_etabli_le'];
...
?>" id="datepicker">
 ...
 <div class="form-inline" > 
  ...
  <span class="input-group-addon">
                ...
                <input name="date_val"  class="form-control" type='text' required="required" value="<?php $date=$result['devis_valable_au'];
...
<div class="col-md-1" >
<select name="statut" id="statut"  class="form-control select2"> 
<?php $req6=mysql_query("select * from masterParametreValeur where idMasterParametre=10");
       while($result6=mysql_fetch_array($req6)) 
       {  if ($result6['id'] == $result['id_statut']) {
...
<div class="form-inline" >  
  ...
  <div class="col-md-8" >
  <input name="prix_ttc" value="<?php echo $result['Prix_ttc'];?>" onchange="updPrice('ttc',this.value);"  class="form-control col-md-10"  required="required" type="number" required name="price" min="0"  step="0.01"  pattern="^\d+(?:\.\d{1,2})?$" onblur="
this.parentNode.parentNode.style.backgroundColor=/^\d+(?:\.\d{1,2})?$/.test(this.value)">
  </div>
  </div>
  </td>
  <td width="25%">
 <div class="form-inline" >  
  ...
  <div class="col-md-8" >
  <input name="prix_ht" value="<?php echo $result['Prix_ht'];?>" onchange="updPrice('ht',this.value);"  class="form-control col-md-10"  required="required" type="number" name="price" min="0" step="0.01"  pattern="^\d+(?:\.\d{1,2})?$" onblur="
this.parentNode.parentNode.style.backgroundColor=/^\d+(?:\.\d{1,2})?$/.test(this.value)">
</div>
  </div>
  </td>
 ...
 <div class="form-inline" >  
  ...
  <div class="col-md-8" >
      <input name="assurance" value="<?php echo $result['assurance'];?>" class="form-control col-md-10"  required="required" type="number"  name="price" min="0" step="0.01"  pattern="^\d+(?:\.\d{1,2})?$" onblur="
this.parentNode.parentNode.style.backgroundColor=/^\d+(?:\.\d{1,2})?$/.test(this.value)">
</div>
    </div>
    </div>
...
<div class="col-md-7" >
<input name="prix_paye"  value="<?php echo $result['montant_paye'];?>" class="form-control col-md-10"  type="number"   name="price" min="0" step="0.01"  pattern="^\d+(?:\.\d{1,2})?$" onblur="
this.parentNode.parentNode.style.backgroundColor=/^\d+(?:\.\d{1,2})?$/.test(this.value)">
</div>
</div>
</td>
...
<div class="form-row" >
  ...
  <label for="exampleInputEmail1">Remarques Pdf Devis</label>
    <textarea name="remarque"   class="form-control" rows="3" ><?php echo $result['rqs'];?></textarea>
  </div>
  <div class="form-group col-md-4"> 
  <label for="exampleInputEmail1">Remarques Privées</label>
  <textarea name="remarque2" class="form-control" rows="3" ><?php echo $result['remarque']; ?></textarea>
</div>
<div class="form-group col-md-4"> 
  <label for="exampleInputEmail1">Instruction</label>
  <textarea name="instruction"   class="form-control" rows="3"><?php echo $result['instruction']; ?></textarea>
  </div>
...
<div class="box box-default">
<div class="box-header with-border">
<h5 class="card-title">Informations Générales Demande
  <div class="box-tools pull-right">
  <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
  </div></h5>
</div>
...
<input name="date_etab"  class="form-control" type='text' required="required" value="<?php $date=$result['etablie_le'];
...
<input name="id_dem" value="<?php echo $result['id_dem'];?>"  type="hidden" class="form-control" >
 ...
 <h5 class="card-title">Déménagement</h5>
    <div class="form-group">
      ...
      <div class="form-group">
       ...
       while($result6=mysql_fetch_array($req6)) 
       { 
        if ($result6['id'] == $result['prest']) {
         echo '<option  value="'.$result['prest'].'" selected="selected">'.$result6['valeur'].' </option>';
        }
        else
        {echo"<option  value='".$result6['id']."'>".$result6['valeur']." </option>"; }
      }
...
<script>
 $(document).ready(function(){
   ...
   function updPrice(typePrix,montant)
   {
     if (typePrix=='ht')
     {

document.contact.prix_ttc.value = Math.round(montant*1.2);
}
if (typePrix=='ttc')
     {
document.contact.prix_ht.value = Math.round(montant/1.2);
}
   }
$(function () {
